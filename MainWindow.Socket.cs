...
public partial class MainWindow
{
    ...
    private readonly object _socketClientsLock = new();
    private readonly List<SocketClientConnection> _socketServerClients = [];

    // Socket 页面的日志缓存。TextBox 负责显示，列表负责保留可裁剪的真实数据源。
    private readonly List<string> _socketLogItems = [];
    ...
    private bool IsSocketServerMode => (ModeComboBox.SelectedItem as ComboBoxItem)?.Tag?.ToString() == "Server";
    ...
    private async void SocketStartButton_Click(object sender, RoutedEventArgs e)
    {
        ...
        if (!TryReadSocketPort(out int port))
        {
            return;
        }
    ...
    private void StartSocketServer(int port)
    {
        ...
        SetSocketRunningState(true, $"服务端已监听 0.0.0.0:{port}");
        AppendSocketLog("系统", $"服务端启动，监听端口 {port}");
    ...
    private async Task StartSocketClientAsync(string host, int port)
    {
        ...
        SetSocketRunningState(true, $"客户端已连接 {address}:{port}");
        AppendSocketLog("系统", $"已连接到服务端 {address}:{port}");
    ...
    {
        while (!cancellationToken.IsCancellationRequested && _socketListener is not null)
        {
            try
            {
                Socket client = await _socketListener.AcceptAsync(cancellationToken);
                string remoteName = client.RemoteEndPoint?.ToString() ?? "未知客户端";
                SocketClientConnection connection = new(client, remoteName);
                ...
                Dispatcher.Invoke(() =>
                {
                    AppendSocketLog("系统", $"客户端已连接：{remoteName}");
            ...
            {
                Dispatcher.Invoke(() => AppendSocketLog("错误", $"接受连接失败：{ex.Message}"));
            }
        }
    }
        ...
        bool isServerClient)
    {
        ...
        try
        {
            while (!cancellationToken.IsCancellationRequested)
            {
                ...
                if (message.Contains("ALIVE_REQ"))
                {
                    ...
                    {
                        Dispatcher.Invoke(() =>
                            AppendSocketLog($"收到 <- {heartbeatPrintCount}次心跳包，此次心跳包为:{Environment.NewLine}",
                                message.TrimEnd()));
                        heartbeatPrintCount = 0;
                    }
                }

                Dispatcher.Invoke(() => AppendSocketLog($"收到 <- {peerName}:{Environment.NewLine}", message.TrimEnd()));
            }
        }
        catch (OperationCanceledException)
        {
        ...
        {
            Dispatcher.Invoke(() => AppendSocketLog("网络", $"{peerName} 连接中断：{ex.Message}"));
        }
        catch (Exception ex)
        {
            Dispatcher.Invoke(() => AppendSocketLog("错误", $"{peerName} 接收失败：{ex.Message}"));
        }
        finally
        {
            if (isServerClient)
            {
                ...
                Dispatcher.Invoke(() =>
                {
                    AppendSocketLog("系统", $"{peerName} 已断开");
                    RefreshSocketClientTargets();
                    UpdateSocketConnectionCount();
                });
            }
    ...
    private async Task SendSocketCurrentMessageAsync()
    {
        ...
        try
        {
            if (IsSocketServerMode)
            {
                ...
                AppendSocketLog($"发送 -> {GetSocketTargetLogName(targets, clients.Length)}", message);
            }
            else
            {
        ...
        {
            AppendSocketLog("错误", $"发送失败：{ex.Message}");
        }
    }
    ...
    private void UpdateSocketConnectionCount()
    {
        if (IsSocketServerMode)
        {
            lock (_socketClientsLock)
            {
                ...
                ConnectionCountTextBlock.Text = $"连接数：{_socketServerClients.Count}";
            }
        }
        else
        {
            ConnectionCountTextBlock.Text = _socketClient?.Connected == true ? "连接数：1" : "连接数：0";
        }
    }
    ...
    {
        if (ClientTargetsListBox is null)
        {
            return;
        }
    ...
    {
        if (TargetDropDownButton is null)
        {
            return;
        }
        ...
        TargetDropDownButton.Content = clients.Length switch
        {
            0 => "暂无客户端",
            _ when selectedCount == clients.Length => $"所有客户端（{clients.Length}）",
            _ when selectedCount == 0 => "未选择客户端",
            1 => clients.First(client => client.IsSelected).DisplayName,
            _ => $"已选择 {selectedCount}/{clients.Length} 个客户端"
        };
    }
    ...
    private static string GetSocketTargetLogName(SocketClientConnection[] targets, int totalClientCount)
    {
        ...
        return targets.Length == 1 ? targets[0].DisplayName : $"{targets.Length} 个客户端";
    }
    ...
    private sealed class SocketClientConnection(Socket socket, string displayName) : INotifyPropertyChanged
    {
