...
namespace ProtocolTestBench
{
    public partial class MainWindow
    {
        ...
        private readonly List<string> _mqttLogItems = new List<string>();
        ...
        private async Task ConnectMqttAsync()
        {
            ...
            try
            {
                ...
                SetMqttConnectedState(true, string.Format("已连接：{0}:{1}，Result={2}", host, port, result.ResultCode));
                AppendMqttLog("连接", string.Format("已连接到 {0}:{1}，ClientId={2}，Result={3}", host, port, clientId, result.ResultCode));
            }
            catch (Exception ex)
            {
                client.Dispose();
                ...
                AppendMqttLog("错误", string.Format("连接 MQTT Broker 失败：{0}", ex.Message));
            }
        }
        ...
        private void WireMqttClientEvents(IMqttClient client)
        {
            ...
            {
                string reason = args.ConnectResult == null ? null : args.ConnectResult.ReasonString;
                if (!string.IsNullOrWhiteSpace(reason))
                {
                    AppendMqttLog("连接", string.Format("Broker 附加信息：{0}", reason));
                }
            }).Task;

            client.DisconnectedAsync += args => Dispatcher.InvokeAsync(() =>
            {
                string detail = args.Exception == null ? null : args.Exception.Message;
            ...
            client.ApplicationMessageReceivedAsync += args => Dispatcher.InvokeAsync(() =>
            {
                ...
                AppendMqttLog(
                    string.Format("收到 <- {0} [QoS {1}, Retain={2}]", topic, (int)message.QualityOfServiceLevel, message.Retain),
                    payload);
            }).Task;
        }
        ...
        private async Task CloseMqttClientAsync()
        {
            if (_mqttClient == null)
            {
                HandleMqttDisconnected("未连接", null);
                return;
            }
        ...
        private async Task SubscribeMqttTopicAsync()
        {
            ...
            try
            {
                    ...
                    !string.Equals(_mqttSubscribedTopic, topic, StringComparison.Ordinal))
                {
                    ...
                    AppendMqttLog("订阅", string.Format("已取消旧主题：{0}", _mqttSubscribedTopic));
                }
                ...
                AppendMqttLog("订阅", string.Format("主题={0}，QoS={1}，Result={2}", topic, (int)qos, result.Items.FirstOrDefault()?.ResultCode));
            }
            catch (Exception ex)
            {
                AppendMqttLog("错误", string.Format("订阅失败：{0}", ex.Message));
            }
        }
        ...
        private async Task UnsubscribeMqttTopicAsync()
        {
            ...
            try
            {
                ...
                _mqttSubscribedTopic = null;
                UpdateMqttSubscriptionState();
                AppendMqttLog("订阅", string.Format("已取消订阅：{0}，Result={1}", topic, result.Items.FirstOrDefault()?.ResultCode));
            }
            catch (Exception ex)
            {
                AppendMqttLog("错误", string.Format("取消订阅失败：{0}", ex.Message));
            }
        }
        ...
        private async Task PublishMqttMessageAsync()
        {
            ...
            try
            {
                ...
                AppendMqttLog(
                    string.Format("发送 -> {0} [QoS {1}, Retain={2}]", topic, (int)qos, retain),
                    string.IsNullOrWhiteSpace(payload) ? "(空消息体)" : payload);
                AppendMqttLog("发布结果", string.Format("Reason={0}，Success={1}", result.ReasonCode, result.IsSuccess));
            }
            catch (Exception ex)
            {
                AppendMqttLog("错误", string.Format("发布失败：{0}", ex.Message));
            }
        }
        ...
        private void HandleMqttDisconnected(string status, string detail)
        {
            _isMqttConnected = false;
            _isMqttDisconnecting = false;
            _mqttSubscribedTopic = null;
            ...
            {
                AppendMqttLog("连接", string.Format("{0}：{1}", status, detail));
            }
            else
            {
        ...
        private void AppendMqttLog(string source, string message)
        {
            ...
            {
                MqttLogTextBox.AppendText(string.Format("{0}{1}", logLine, Environment.NewLine));
            }
