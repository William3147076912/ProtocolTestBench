...
namespace ProtocolTestBench
{
    public partial class MainWindow
    {
        ...
        private readonly List<string> _stompLogItems = new List<string>();
        ...
        private void ConnectStomp()
        {
            ...
            try
            {
                ...
                StompConnectionSettings settings = new StompConnectionSettings
                {
                    ClientId = clientId,
                ...
                ConnectionFactory connectionFactory = new ConnectionFactory(brokerUri.ToString(), settings);
                ...
                SetStompConnectedState(true, string.Format("已连接：{0}", brokerUri));
                AppendStompLog("连接", string.Format("已连接到 {0}，ClientId={1}", brokerUri, clientId));
            }
            catch (Exception ex)
            {
                CloseStompClient();
                SetStompConnectedState(false, "连接失败");
                AppendStompLog("错误", string.Format("连接 STOMP Broker 失败：{0}", ex.Message));
            }
        }
        ...
        private void CloseStompClient()
        {
            ...
            {
                AppendStompLog("错误", string.Format("关闭订阅失败：{0}", ex.Message));
            }
            ...
            {
                AppendStompLog("错误", string.Format("关闭发送器失败：{0}", ex.Message));
            }
            ...
            {
                AppendStompLog("错误", string.Format("关闭会话失败：{0}", ex.Message));
            }
            ...
            {
                AppendStompLog("错误", string.Format("关闭连接失败：{0}", ex.Message));
            }
        ...
        {
            if (_stompSession == null || !_isStompConnected)
            {
            ...
            try
            {
                ...
                _stompSubscribedDestinationLabel = string.Format("{0}:{1}", GetSelectedStompDestinationType(StompSubscribeTypeComboBox), destinationName);
                UpdateStompSubscriptionState();
                AppendStompLog("订阅", string.Format("已订阅 {0}，Ack={1}", _stompSubscribedDestinationLabel, GetSelectedStompAckMode()));
            }
            catch (Exception ex)
            {
                AppendStompLog("错误", string.Format("订阅失败：{0}", ex.Message));
            }
        }
        ...
        private void UnsubscribeStompDestination()
        {
            ...
            try
            {
                _stompConsumer.Listener -= OnStompMessageReceived;
                _stompConsumer.Close();
                AppendStompLog("订阅", string.Format("已取消订阅：{0}", _stompSubscribedDestinationLabel));
            }
            catch (Exception ex)
            {
                AppendStompLog("错误", string.Format("取消订阅失败：{0}", ex.Message));
            }
            finally
            {
                _stompConsumer = null;
        ...
        private void OnStompMessageReceived(IBytesMessage message)
        {
            Dispatcher.Invoke(() =>
            {
                try
                {
                    string destination = message.FromDestination == null ? null : message.FromDestination.PhysicalName;
                    if (string.IsNullOrWhiteSpace(destination))
                    {
                        destination = message.StompDestination == null ? null : message.StompDestination.PhysicalName;
                    }

                    if (string.IsNullOrWhiteSpace(destination))
                    {
                        destination = _stompSubscribedDestinationLabel;
                    }
                    ...
                    AppendStompLog(string.Format("收到 <- {0}", destination), payload);

                    if (GetSelectedStompAckMode() != AcknowledgementMode.AutoAcknowledge)
                    {
                        // 非自动确认模式下，这里收到并记录后立刻 ACK，避免 broker 重复投递。
                        message.Acknowledge();
                        AppendStompLog("ACK", string.Format("已确认消息：{0}", message.StompMessageId));
                    }
                }
                catch (Exception ex)
                {
                    AppendStompLog("错误", string.Format("处理收到的 STOMP 消息失败：{0}", ex.Message));
                }
            });
        }
        ...
        {
            if (_stompSession == null || _stompProducer == null || !_isStompConnected)
            {
                AppendStompLog("提示", "STOMP 客户端尚未连接。");
                return;
            }
            ...
            try
            {
                ...
                AppendStompLog(string.Format("发送 -> {0}:{1}", GetSelectedStompDestinationType(StompPublishTypeComboBox), destinationName),
                    string.IsNullOrWhiteSpace(payload) ? "(空消息体)" : payload);
            }
            catch (Exception ex)
            {
                AppendStompLog("错误", string.Format("发送失败：{0}", ex.Message));
            }
        }
        ...
        {
            bool hasSubscription = _stompConsumer != null;
        ...
        {
            if (GetSelectedStompDestinationType(comboBox) == "Topic")
            {
                return session.GetTopic(destinationName);
            }

            return session.GetQueue(destinationName);
        }
        ...
        private void AppendStompLog(string source, string message)
        {
            ...
            {
                StompLogTextBox.AppendText(string.Format("{0}{1}", logLine, Environment.NewLine));
            }
