...
public partial class MainWindow
{
    ...
    private readonly List<string> _cfxLogItems = [];
    ...
    private readonly List<AmqpChannelAddress> _cfxPublishChannels = [];
    private readonly List<AmqpChannelAddress> _cfxSubscribeChannels = [];
    ...
    private void OpenCfxEndpoint()
    {
        ...
        if (!Uri.TryCreate(brokerText, UriKind.Absolute, out Uri? brokerUri) ||
        ...
        if (!string.IsNullOrWhiteSpace(requestUriText) &&
            (!Uri.TryCreate(requestUriText, UriKind.Absolute, out Uri? requestUri) ||
        ...
        try
        {
            ...
            {
                Uri endpointRequestUri = new(requestUriText);
            ...
            if (!string.IsNullOrWhiteSpace(publishAddress))
            {
                ...
                AmqpChannelAddress publishChannel = new()
                {
                    Uri = brokerUri,
            ...
            if (!string.IsNullOrWhiteSpace(subscribeAddress))
            {
                ...
                AmqpChannelAddress subscribeChannel = new()
                {
                    Uri = brokerUri,
            ...
            SetCfxRunningState(true, $"端点已打开：{handle}");
            AppendCfxLog("系统", $"已打开 CFX 端点，Broker={brokerUri}");
        ...
        catch (Exception ex)
        {
            ...
            AppendCfxLog("错误", $"打开 CFX 端点失败：{ex.Message}");
    ...
    private void CloseCfxEndpoint()
    {
        ...
        try
        {
            foreach (AmqpChannelAddress subscribeChannel in _cfxSubscribeChannels.ToArray())
            {
                ...
                {
                    AppendCfxLog("错误", $"关闭订阅通道失败：{FormatCfxAddress(subscribeChannel)}，{ex.Message}");
                }
            }
            ...
            foreach (AmqpChannelAddress publishChannel in _cfxPublishChannels.ToArray())
            {
                ...
                {
                    AppendCfxLog("错误", $"关闭发布通道失败：{FormatCfxAddress(publishChannel)}，{ex.Message}");
                }
            }
        ...
        {
            AppendCfxLog("错误", $"关闭 CFX 端点时发生错误：{ex.Message}");
        }
        finally
        {
    ...
    {
        if (_cfxEndpoint is null || !_isCfxOpen)
        {
        ...
        try
        {
            ...
            AppendCfxLog($"发送 -> {CfxPublishAddressTextBox.Text.Trim()}", envelope.ToJson(true));
        }
        catch (Exception ex)
        {
            AppendCfxLog("错误", $"发送 CFX 消息失败：{ex.Message}");
        }
    }
    ...
    private void LoadCfxSampleMessage()
    {
        ...
        CFXEnvelope envelope = new(new LogEntryRecorded
        {
            Importance = LogImportance.Information,
    ...
    private void LoadCfxRequestResponseSamples()
    {
        ...
        CFXEnvelope requestEnvelope = new(new AreYouThereRequest
        {
            CFXHandle = handle
        })
        {
        ...
        CFXEnvelope responseEnvelope = new(new AreYouThereResponse
        {
    ...
    private async Task ExecuteCfxPointToPointRequestAsync()
    {
        ...
        if (!Uri.TryCreate(targetUri, UriKind.Absolute, out Uri? parsedTargetUri) ||
        ...
        try
        {
            ...
            AppendCfxLog($"请求 -> {targetUri}", requestEnvelope.ToJson(true));
            CFXEnvelope responseEnvelope = await endpoint.ExecuteRequestAsync(targetUri, requestEnvelope);
            AppendCfxLog($"响应 <- {targetUri}", responseEnvelope.ToJson(true));
        }
        catch (Exception ex)
        {
            AppendCfxLog("错误", $"执行点对点请求失败：{ex.Message}");
        }
    }
    ...
    private void CfxEndpoint_OnCFXMessageReceived(AmqpChannelAddress source, CFXEnvelope message)
    {
        ...
        Dispatcher.Invoke(() => { AppendCfxLog($"收到 <- {FormatCfxAddress(source)}", message.ToJson(true)); });
    }
    ...
    private void CfxEndpoint_OnMalformedMessageReceived(AmqpChannelAddress source, string message)
    {
        ...
        Dispatcher.Invoke(() => { AppendCfxLog($"异常消息 <- {FormatCfxAddress(source)}", message); });
    }
    ...
    private CFXEnvelope CfxEndpoint_OnRequestReceived(CFXEnvelope request)
    {
        ...
        return Dispatcher.Invoke(() =>
        {
            ...
            {
                CFXEnvelope fallback = CreateDefaultCfxResponseEnvelope(request, $"自动响应 JSON 解析失败：{ex.Message}");
        ...
        Exception errorException)
    {
        ...
        Dispatcher.Invoke(() =>
        {
            string detail = string.IsNullOrWhiteSpace(errorInformation) ? uri.ToString() : $"{uri}，{errorInformation}";
            if (errorException is not null)
            {
                detail = $"{detail}，{errorException.Message}";
            }

            AppendCfxLog("连接", $"{eventType}，Spool={spoolSize}，{detail}");
        });
    }
    ...
    private static string FormatCfxAddress(AmqpChannelAddress address)
    {
        ...
        return $"{address.Uri}{address.Address}";
    }
    ...
    private void AppendCfxLog(string source, string message)
    {
        ...
        {
            CfxLogTextBox.AppendText($"{logLine}{Environment.NewLine}");
        }
